...
const keys = ['hero', 'founderStory', 'waysToExperience', 'availableTours', 'howHosted', 'guestStory', 'reviewsAndTrust', 'planningProcess', 'stories', 'finalInvitation'];
...
await assert.rejects(
  loadHomepageContent({
    localContent,
    env: {SANITY_STUDIO_PROJECT_ID: 'project-123'},
    fetchImpl: async () => new Response(JSON.stringify({result: sections.slice(0, 9)}), {status: 200}),
  }),
  /exactly 10 sections/,
);
