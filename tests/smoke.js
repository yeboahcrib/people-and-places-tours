...
(async () => {
  ...
  await withPage(browser, '/index.html', async page => {
    ...
    assert(cards === 6, `expected 6 homepage trip cards, got ${cards}`);
    ...
    const lazyInstagramImages = await page.locator('.ig-photo img[loading="lazy"][width="600"][height="600"]').count();
    assert(lazyInstagramImages === 6, `expected 6 lazy Instagram strip images, got ${lazyInstagramImages}`);
