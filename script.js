...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /*
   * The packages grid is built by the static build and left alone here.
   *
   * This used to rebuild every card from window.PEOPLE_PLACES_TOURS after load,
   * which silently replaced the CMS-authored cards the build had just rendered
   * with values reconstructed from the committed catalogue. Measured on the
   * live site, ten Sanity card photographs were served in the HTML and then
   * discarded about a second later; prices, titles and badges were exposed to
   * the same override and agreed only because tours.js was kept in step by hand.
   *
   * Filtering never needed the rebuild: every server-rendered card already
   * carries data-tour-slug, data-destination and data-categories, which is all
   * applyFilters reads.
   */
